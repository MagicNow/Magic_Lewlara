...
@section('content')
    ...
    <div class="container-fluid conteudo-blog">
        <div class="row-fluid">
            <h4 class="titulo-conteudo" title="{{$tag_slug}}"> {{$tag_slug}}</h4>
                    ...
                    @foreach($posts_tag as $post_tag)
                        <div class="media">                              
                            <div class="media-left media-top">
                                <a class="link_thumb" href="{{action('blog\HomeBlogController@click_interna',array($cliente_default->slug,$post_tag->slug))}}" title="@if($post_tag->midia->first()) {{ $post_tag->midia->first()->titulo}} @endif">
                                    <img class="media-object img-conteudo-post-destaque" src="
                                    @if($post_tag->midia->first())
                                    {{ URL::to('/').'/'.$post_tag->midia->first()->imagem}} 
                                    @endif" alt="...">
                                </a>
                            </div>
                            <div class="media-body">
                                ...
                                <div class="btn btn-amarelo btn-medio align-center btn-top-post">
                                    <img src="{{ URL::to('/img/ico_categoria.png') }}" class="logo-cliente-slider"> 
                                        ...
                                        @foreach ($post_tag->tag as $key => $tag)
                                            @if($key < count($post_tag->tag)-1)
                                ...
                                @if($post_tag->concorrente->first())
                                    <div class="btn btn-cinza btn-medio align-center btn-top-post">
                                        <a class="link-post" href="{{ action('blog\HomeBlogController@click_concorrente',array($cliente_default->slug,$post_tag->concorrente->first()->id)) }}">
                                        CONCORRENTE: 
                                                {{ mb_strtoupper($post_tag->concorrente->first()->name) }}
                                            </a>
                                    </div>
                                @endif
                            </div>
